...
Introduce the main concept of the lesson in a few sentences. Provide a
simple example or illustration if appropriate.
...
Present the primary topic or tool being taught. Include an example
demonstrating how it is used. Add a brief note or tip if needed.
...
Double-ended queue • Fast insert/remove at both ends • More flexible
than list for queue/stack behavior
...
Arrays: contiguous memory, fixed type Lists: dynamic, flexible, allow
mixed types
...
Set membership: O(1) average • Dict key lookup: O(1) average • Sorting:
O(n log n)
