...
| Software Testing is a process of verifying that a software application
  works as expected
| and meets the user’s requirements. It helps identify bugs, errors, and
  performance issues
...
| Term            | Meaning                                            |
+=================+====================================================+
| Bug/Defect      | A flaw that causes software to behave              |
|                 | unexpectedly.                                      |
+-----------------+----------------------------------------------------+
| Error           | A human mistake that introduces a defect.          |
+-----------------+----------------------------------------------------+
| Test Case       | A set of conditions or inputs to check a specific  |
|                 | feature.                                           |
+-----------------+----------------------------------------------------+
| Test Plan       | A document that outlines scope, approach, and      |
|                 | schedule of testing.                               |
+-----------------+----------------------------------------------------+
...
| Level         | Focus                        | Example               |
+===============+==============================+=======================+
| Unit Testing  | Testing individual functions | Function add(a, b)    |
|               | or modules.                  |                       |
+---------------+------------------------------+-----------------------+
| Integration   | Testing the interaction      | user_login() calling  |
| Testing       | between modules.             | db_connect()          |
+---------------+------------------------------+-----------------------+
| System        | Testing the whole            | Full web app test     |
| Testing       | application end-to-end.      |                       |
+---------------+------------------------------+-----------------------+
| Acceptance    | Validating with client       | UAT before deployment |
...
| Goal: Verify that individual parts (functions, methods) work as
  expected.
...
Goal: Confirm the system meets the client’s or stakeholder’s
requirements.
...
+-------+--------------------+---------+--------------+------------+--------+
| Test  | Description        | Input   | Expected     | Actual     | Result |
| ID    |                    |         | Output       | Output     |        |
+=======+====================+=========+==============+============+========+
| TC_01 | Add positive       | (2,3)   | 5            | 5          | Pass   |
|       | numbers            |         |              |            |        |
+-------+--------------------+---------+--------------+------------+--------+
| TC_02 | Add negative       | (-1,-1) | -2           | -2         | Pass   |
|       | numbers            |         |              |            |        |
+-------+--------------------+---------+--------------+------------+--------+
...
1. Software testing ensures: A. Performance only B. Code compilation
   only C. Software works as expected D. System design ➤ **Answer: C**

2. A test case includes: A. Input and expected output B. Source code
   only C. Documentation only D. UI design ➤ **Answer: A**
...
Unit testing verifies: A. Database connections B. Entire system behavior
C. Individual functions or classes D. User interface ➤ **Answer: C**

Integration testing ensures: A. Code style B. Module interactions C.
Syntax checking D. User experience ➤ **Answer: B**
...
Non-functional testing includes: A. Integration Testing B. Unit Testing
C. Load Testing D. Acceptance Testing ➤ **Answer: C**

Regression testing is done to: A. Find new bugs B. Ensure old features
work after changes C. Optimize UI D. Speed up performance ➤ **Answer:
B**
...
Automation testing helps: A. Reduce human error B. Slow down testing C.
Replace debugging D. Skip CI/CD ➤ **Answer: A**

Which tool is used for web testing? A. pytest B. Selenium C. unittest D.
coverage ➤ **Answer: B**
...
Test cases should be: A. Random B. Repeatable and clear C. Unstructured
D. Unplanned ➤ **Answer: B**
...
| Type      |                                        |                   |
+===========+========================================+===================+
| Syntax    | Violates Python grammar rules          | ``print "Hello"`` |
| Error     |                                        |                   |
+-----------+----------------------------------------+-------------------+
| Runtime   | Happens during execution               | ``10 / 0``        |
| Error     |                                        |                   |
+-----------+----------------------------------------+-------------------+
| Logical   | Program runs but produces incorrect    | wrong area        |
| Error     | output                                 | formula           |
+-----------+----------------------------------------+-------------------+
...
+------------+-------------------------+-------------------------------+
| Who        | QA/Test Engineer        | Developer                     |
| performs   |                         |                               |
+------------+-------------------------+-------------------------------+
